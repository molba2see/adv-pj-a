    ...
    def __init__(
        ...
        **kwargs,
        ...
        if lora:
            ...
            # MotionGPT's default backbone is T5 (encoder-decoder), not a
            # decoder-only causal LM.  The explicit target modules also make
            # this work across PEFT versions that cannot infer T5 targets.
            peft_config = LoraConfig(
                bias="none",
                task_type=TaskType.SEQ_2_SEQ_LM if self.lm_type == 'encdec'
                else TaskType.CAUSAL_LM,
                r=8,
                lora_alpha=16,
                lora_dropout=0.05,
                target_modules=["q", "v"],
                # MotionGPT adds motion tokens immediately before applying
                # LoRA.  Their embeddings must remain trainable.
                modules_to_save=["shared", "lm_head"] if self.lm_type == 'encdec' else None)
